...
const SundryCreditorsAlter = () => {
  ...
  useEffect(() => {
    ...
    const loadSuppliers = async () => {
      try {
        ...
        console.log('API Response:', result.data); // Inspect the structure of the data
  ...
  // Handle input changes in bank subform
  ...
  const calculateTotals = (forexSubForm) => {
    let totalForexAmount = 0;
    let totalAmount = 0;

    // Iterate over each row in forexSubForm to calculate totals
    const updatedForexSubForm = forexSubForm.map(row => {
      ...
      return {
        ...row,
        referenceAmount: referenceAmount.toLocaleString('en-IN', {
  ...
  // Calculate totals outside the state update
  const updatedForexSubForm = sundryCreditor.forexSubForm.map((row, i) => {
    if (i === index) {
      return {
        ...row,
        [name]: value,
  ...
  if (name === 'forexCurrencyType' && index === 0) {
    ...
    updatedForexSubForm.forEach((row, i) => {
      if (i > 0) {
        updatedForexSubForm[i] = {
          ...row,
          forexCurrencyType: value,
          forexCurrencySymbol: currencySymbol,
        };
      }
    });
  }
  ...
  if (name === 'exchangeRate' && index === 0) {
    ...
    if (updatedForexSubForm.length === 1) {
      ...
      const selectedCurrency = currencySuggestion.find(
        currency =>
          currency.forexCurrencyName.toLowerCase() === forexCurrencyType.toLowerCase()
      );
      ...
      updatedForexSubForm.push({
        ...
        exchangeRate: value,
        referenceAmount: '',
        referenceCreditOrDebit: sundryCreditor.creditOrDebit,
      });
    }
  }
  ...
  if (
    ...
    updatedForexSubForm.push({
      ...
      forexCurrencySymbol,
      forexAmount: '',
      exchangeRate: updatedForexSubForm[0].exchangeRate,
      referenceAmount: '',
      referenceCreditOrDebit: sundryCreditor.creditOrDebit,
    });
  }

  // Calculate totals after the state is updated to prevent re-render issues
  const totals = calculateTotals(updatedForexSubForm);

  setSundryCreditor(prevState => ({
    ...prevState,
    forexSubForm: updatedForexSubForm,
    totalForexAmount: totals.totalForexAmount,
  ...
  const prepareDataForBackend = sundryCreditor => {
    return {
      ...
      sundryCreditorForexDetails: sundryCreditor.forexSubForm
        .filter(forex => forex.forexDate.trim() !== '') // Filter out rows with empty forexDate
        .map(forex => ({
          ...forex,
  ...
  const handleSubmit = async e => {
    ...
    const preparedData = prepareDataForBackend(sundryCreditor);

    // Confirmation dialog
    const userConfirmed = window.confirm('Do you want to confirm this submit?');
    if (userConfirmed) {
      try {
        const response = await updateSundryCreditorMaster(datas, preparedData);
        console.log('Sundry Creditor altered successfully!', response.data);

        // Refocus on the first input if present
        if (inputRefs.current[0]) {
          inputRefs.current[0].focus();
        }
      } catch (error) {
        console.error('Error updating sundry creditor master:', error);
      }
    }
    navigate(-1);
  };
        ...
        <form action="" className="border border-slate-500 w-[48.5%] h-[92.7vh]" onSubmit={handleSubmit}>
          ...
          <div className="flex">
            <div className="text-sm pl-1 mb-1">
              <label htmlFor="billWiseStatus" className="ml-2 mr-[155px]">
                Bill-Wise Status
              </label>
              <span>:</span>
              <input
          ...
          <div className="text-sm pl-1 mb-1 flex">
            ...
            <input
              ...
              className="w-[200px] ml-2 h-5 pl-1 font-medium text-sm focus:bg-yellow-200 focus:outline-none focus:border-blue-500 focus:border"
              autoComplete="off"
            />
          </div>
          <div className="text-sm pl-1 mb-1 flex">
            <label htmlFor="openingBalance" className="w-[22%] ml-2">
              Opening Balance
            </label>
            (
            <input
