...
const SundryCreditorsDisplay = () => {
  ...
  const handleKeyDownForex = (e, index) => {
    ...
    if (key === 'Enter') {
      ...
      if (sundryCreditor.forexApplicable === 'no' && nextField >= 4) {
        // Skip the forex fields if forexApplicable is 'no'
        nextField = 9; // Adjust this based on your table structure
      } else if (sundryCreditor.forexApplicable === 'yes' && nextField >= 7) {
      ...
      if (e.target.value.trim() === '' && index > 0) {
        ...
        if (sundryCreditor.forexApplicable === 'no' && prevField >= 4) {
          // Skip back to the first column if previous is a forex field
          inputRefsForex.current[0].focus();
        } else if (sundryCreditor.forexApplicable === 'yes' && prevField >= 7) {
