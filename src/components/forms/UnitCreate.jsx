  ...
  const handleKeyDown = (e, index) => {
    ...
    if (key === 'Enter'){
      ...
      if (e.target.value.trim() !== ''){
        ...
        } else if (e.target.name === 'numberOfDecimalPlaces'){
      ...
      if (filteredSuggestion.length > 0) {
        e.preventDefault();
        setHighlightedSuggestionUnitCode((prev) => Math.min(prev + 1, filteredSuggestion.length - 1));
      }
    } else if (key === 'ArrowUp') {
      if (filteredSuggestion.length > 0) {
        e.preventDefault();
        setHighlightedSuggestionUnitCode((prev) => Math.max(prev - 1, 0));
      }
    }
  };
